...
// import { TimetableSaveToCalender } from "../components/timetable-save-to-calender";
...
// 	if (!year || !programme || !level || !sem || !_day) {
...
// 	if (isNaN(day) || isNaN(levelNum) || isNaN(semNum)) {
...
export const meta: MetaFunction<typeof loader> = ({ data }) => {
	const description = `See timetable for ${data?.programme}, L${data?.level} for the year ${data?.year}`;
		...
		return [ // ✅ This should be an array for your Remix version
		{ title: `Timetable | ${data?.school.shortName} ✽ ttucampus` },
