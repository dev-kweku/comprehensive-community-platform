...
// 	return [
// 		{
// 			title: `@${data?.post.user.username} posted in Discussions | ${data?.meta.shortName} ✽ ttucampus`,
// 		},
...
export const loader = async ({ request, params }: LoaderFunctionArgs) => {
	...
	return json({
		comments: await includeVotes(comments, request),
		school: values.meta(), // ✅ Changed from 'meta' to 'school'
		post: { ...post, vote },
	});
};
...
export const meta: MetaFunction<typeof loader> = ({ data }) => {
	...
	return [
		{
			title: `@${data?.post.user.username} posted in Discussions | ${data?.school.shortName} ✽ ttucampus`, // ✅ Fixed: data?.school.shortName
		},
		{ name: "description", content: `${summary}…` },
	];
};
